...
\end{lstlisting}
The above is an example handler \code{h} that handles two operations \code{Get} and \code{Put}.
...
\begin{example}[The ``positive'' interpretation]\label{ex:positive}
...
The \code{positive} handler always picks \code{true}. To implement this behaviour, we invoke the continuation once with argument \code{true}. The value \code{true} becomes the concrete output of \code{do Choose} in the computation:
\begin{lstlisting}[style=links]
handler positive(m) {
...
The two computations are symmetrical. The input parameter \code{n} is the number of sticks left in the heap. First, Alice checks whether there are any sticks left, if there is not then she declares \type{Bob} the winner, otherwise she performs her move and then she gives the turn to Bob.
...
\end{lstlisting}
The operation \code{Move} is handled uniformly, that is, independent current game configuration the handler always invokes the continuation \code{k} with 1.
