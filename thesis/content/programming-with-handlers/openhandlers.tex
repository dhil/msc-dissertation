...
\begin{itemize}
  \item $\code{putLogger} : (() \xrightarrow{ \{\type{Put}:s \to () \; | \; \rho_1\}\;} s) \to () \xrightarrow{ \{ \type{LogPut}: s \to (),\type{Put}:s \to () \; | \; \rho_1\}\;} s$
...
The state handler enable us to implement abstract, stateful handlers that employ the state operations. In this section we will demonstrate how to implement a simple, but highly modular, backtracking parser-handler that interprets parser combinators.